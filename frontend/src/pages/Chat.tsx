...
export default function Chat() {
  ...
  return (
    <div className="h-screen bg-[#0f0a1a] flex flex-col">
      ...
      >
        <div className="max-w-3xl mx-auto w-full px-3 sm:px-5 py-3 space-y-[3px]">
            ...
            return (
              ...
              >
                <div className={`relative max-w-[75%] sm:max-w-[65%] ${isOwn ? "ml-12" : "mr-12"}`}>
                  {/* Bubble */}
                  <div
                    className={`relative px-[9px] pt-[6px] pb-[7px] shadow-sm ${
                      isOwn
                        ? `bg-liner-to-br from-violet-600 to-purple-700 text-white ${isFirstInGroup ? "rounded-tl-lg rounded-tr-[3px] rounded-bl-lg rounded-br-lg" : "rounded-lg"}`
                        : `bg-[#1c1528] text-[#e9edef] border border-violet-500/10 ${isFirstInGroup ? "rounded-tl-[3px] rounded-tr-lg rounded-bl-lg rounded-br-lg" : "rounded-lg"}`
                    }`}
                  >
                    {/* Sender name for others — only on first in group */}
                    {!isOwn && isFirstInGroup && (
                      <p className="text-[12.5px] font-semibold mb-[2px]" style={{ color: getNameColor(msg.username) }}>
                        {msg.username}
                      </p>
                    )}
                    ...
                    <div className="flex flex-wrap items-end">
                      <span className="text-[14.2px] leading-[19px] break-words whitespace-pre-wrap">{msg.text}</span>
                      {/* Invisible spacer so text wraps before the timestamp */}
                      <span className="inline-block w-[68px] shrink-0" />
                    </div>
                    ...
                    <div className="absolute bottom-[5px] right-[7px] flex items-center gap-1">
                      ...
                      {isOwn && (
                        <svg className="w-[16px] h-[11px] text-violet-300/70" viewBox="0 0 16 11" fill="none">
      ...
      <div className="shrink-0 bg-[#13111c] border-t border-violet-500/10 px-3 sm:px-4 py-2.5">
        <form onSubmit={handleSendMessage} className="max-w-3xl mx-auto w-full flex items-center gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-[#1c1528] rounded-lg text-[#e9edef] placeholder-violet-300/30 text-[15px] focus:outline-none focus:ring-1 focus:ring-violet-500/30 transition-colors duration-150 border border-violet-500/10"
            autoComplete="off"
          />
          <button
            type="submit"
            disabled={!inputValue.trim()}
            className="shrink-0 w-[42px] h-[42px] flex items-center justify-center rounded-full bg-violet-600 text-white hover:bg-violet-500 transition-colors duration-150 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shadow-lg shadow-violet-500/20"
          >
